...
private val dfs = MutableList(21) { MutableList(21) { MutableList(21) { 0 } } }
fun main() {
    while (true) {
...
private fun w(a: Int, b: Int, c: Int): Int {
    return if (a <= 0 || b <= 0 || c <= 0) {
        ...
        dfs[20][20][20] = w(20, 20, 20)
        dfs[20][20][20]
    } else if (dfs[a][b][c] != 0) { // 여기서 값을 다시 계산히자 않고 저장되어 있는 값들을 불러옴
        dfs[a][b][c]
    } else if (b in (a + 1)..<c) {
        dfs[a][b][c] = w(a, b, c - 1) + w(a, b - 1, c - 1) - w(a, b - 1, c)
        dfs[a][b][c]
    } else {
        dfs[a][b][c] =
