...
// 골드 2 가장 긴 증가하는 부분 수열 3
// 이분 탐색을 배열에서 숫자 대치 하는데에서 사용함 -> 대치 하는 이유는 나중에 더 큰 숫자가 들어 갈 수 있게 하기 위함
// 길이만 구하는데 사용할 수 있음
// lis가 비어 있으면 맨 처음 숫자를 추가, 그 다음에 각 값을 lis와 비교 하는데
// 이분 탐색을 통하여 값을 비교하여 lis에서 찾는 값보다 같거나 큰 숫자가 나오는 위치에 찾는 값을 대치함 -> "현재 길이를 유지하면서, 끝값을 더 작게 만들어 뒤에 더 많은 숫자가 올 수 있는 가능성을 열어두겠다."
// "현재의 길이는 유지하되, 더 좋은 조건(작은 끝값)으로 갱신하여 미래에 더 길게 이어질 가능성을 높이기 위함"
fun main() {
    val n = readln().toInt()
